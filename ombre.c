...
** Last update Tue May 27 16:23:23 2014 dylan manin
*/
...
int		darken(int color, int x)
{
  ...
  g < 0.0001 ? g = 0.0 : 0;
  b < 0.0001 ? b = 0.0 : 0;
  return (get_color((int) b, (int) g, (int)r));
}
  ...
  while (mw.tab_obj[j].obj != 0)
    {
      ...
      if (mw.tab_obj[j].obj == 2 &&
	  mw.tab_obj[j].trans.z > mw.tab_lum[i].xyz.z)
	return (1);
